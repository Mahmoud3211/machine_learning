...
def summarize_lines(lines, selected_y):
    lines = filter_irrelevant_lines(sort_lines(lines))
    x = 0
    info_array = []
    slope_array = []
    ...
    lines = np.array(lines)[np.argsort(info_array)]
    info_array = np.array(info_array)[np.argsort(info_array)]
    # slope_array = np.array(slope_array)[np.argsort(info_array)]
    # lines = np.array(lines)[np.argsort(info_array)]
...
test_images = glob.glob('D:\Work\Wakeb\power_pipes\X-ray\power pipes pic\Front\*\*.bmp')
results_path = os.path.join(os.getcwd(), 'results2')
test_path = os.path.join(os.getcwd(), 'test')
for z, image in enumerate(test_images):
    ...
    # cv2.imwrite(os.path.join(results_path, f'{z}_res.jpg'), cv2.resize(lineImage, (800, 600)))
    # cv2.waitKey(0)
    # quit()
    ...
    cv2.imwrite(os.path.join(results_path, f'{z}_wall_thickness.jpg'), cv2.resize(clineImage, (800, 600)))
    cv2.imwrite(os.path.join(results_path, f'{z}_final result.jpg'), cv2.resize(result_image, (800, 600)))
    print('[+] Number of images processed:', z+1)
print('[!] Finished <|*_*|>')
# cv2.waitKey(0)
